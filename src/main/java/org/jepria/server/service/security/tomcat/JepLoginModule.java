...
import static org.jepria.compat.server.JepRiaServerConstant.LOGIN_SUFFIX_FOR_HASH_AUTHORIZATION;
...
public class JepLoginModule implements LoginModule {
  ...
  public boolean login() throws LoginException {
    ...
    try {
      fillPrincipals(connection, operatorId);
   ...
   */
  private Integer doJepAuthentication(Connection connection, String username, String password, boolean withHash) throws FailedLoginException {
    logger.trace("BEGIN doJepAuthentication(" + username + ", " + password + ")");
    Integer operatorId;
    //Проверим логин/пароль пользователя.
    try {
      operatorId = pkg_Operator.logon(connection, username, withHash ? null : password, withHash ? password : null);
      connection.commit();
    } catch (SQLException e) {
  ...
  private List<String> getRoles(Connection connection) throws SQLException {
    logger.trace("BEGIN getRoles()");
    if (this.roles == null) {
      roles = pkg_Operator.getRoles(connection, this.username);
    }
   ...
   * @throws SQLException
   */
  private void fillPrincipals(Connection connection, Integer operatorId) throws SQLException {
